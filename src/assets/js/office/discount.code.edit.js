...
$(document).ready(function() {
    ...
    $('.change-discount-state').click(function() {
        if(active_status == "enable") {
            $('.active-staus').text("Disable");
            $('.discount-name').text($('#discount-name').val());
            $('#status-modal .confirm').text("Disable");
        } else {
            $('.active-staus').text("Enable");
            $('.discount-name').text($('#discount-name').val());
            $('#status-modal .confirm').text("Enable");
        }
        $('#status-modal').modal("show");
    });
    $('#status-modal .confirm').click(function() {
        if(active_status == "enable") {
            active_status = "disable";
            $('#info-card .badge').removeClass("bg-success-lt");
            $('#info-card .badge').addClass("bg-secondary-lt");
            $('.status-text').text("Expired");
            $('.change-discount-status-text').text("Enable discount");
            $('.active-status-text').text("active now and will have no end date. All unsaved changes will be lost.");
            $('.active-status').text("Enable");
            $('.start-date-text').text($('.start-date-text').text() + " to " + currentTime());
            $('#status-modal .confirm').removeClass("btn-danger");
            $('#status-modal .confirm').addClass("btn-success");
            $('.i-con-close').addClass("i-con-ok");
            $('.i-con-close').removeClass("i-con-close");
            notification("Discount code disabled");
        }
        else {
            active_status = "enable";
            $('#info-card .badge').addClass("bg-success-lt");
            $('#info-card .badge').removeClass("bg-secondary-lt");
            $('.status-text').text("Active");
            $('.change-discount-status-text').text("Disable discount");
            $('.active-status-text').text("expire now and all unsaved changes will be lost.");
            $('.active-status').text("Disable");
            $('.start-date-text').text($('.start-date-text').text().substring(0, $('.start-date-text').text().indexOf("to")));
            $('#status-modal .confirm').addClass("btn-danger");
            $('#status-modal .confirm').removeClass("btn-success");
            $('.i-con-ok').addClass("i-con-close");
            $('.i-con-ok').removeClass("i-con-ok");
            notification("Discount code enabled");
        }
    });
    $('#delete-modal .confirm').click(function() {
        $('footer .save').attr("disabled", true);
        $('#ghost .all-link')[0].click();
        notification($('#discount-name').val() + " discount code deleted successfully.");
    });
    $('footer .save').click(function() {
        $(this).attr("disabled", true);
        notification($('#discount-name').val() + " discount code updated successfully.");
        $('#ghost .all-link')[0].click();
    });
    $('input').on('change', function() {$('footer .save').attr("disabled", false)});
    $('input').on('keyup', function() {$('footer .save').attr("disabled", false)});
    ...
    $('.limit-total').on('keyup', function() {change_Limit_total();});
    $('.limit-times').on('click', function() {change_Limit_total();});
    $('.one-per-customer').on('click', function() {change_Limit_total();});
...
function currentTime() {
    return $('.end-date').val().substring(0, $('.end-date').val().length - 6) + " at " + $('#select2-end-time-container').text() + " " + $('.end-flag').val();
}
function setTime() {
    ...
    $('.end-date').val(month[today[0] - 1] + ' ' + today[1] + ', ' + today[2]);
...
function change_Limit_total() {
    if($('.limit-times').prop("checked")) {
        if($('.limit-total').val() == "") {
            $('.user-limit-text').text("");
            if($('.one-per-customer').prop("checked")) $('.user-limit-text').text("One use per customer");
        }
        else {
            if($('.limit-total').val() == "1") $('.user-limit-text').text("Limit of 1 use");
            else $('.user-limit-text').text("Limit of " + $('.limit-total').val() + " uses");
            if($('.one-per-customer').prop("checked")) $('.user-limit-text').text($('.user-limit-text').text() + ", one per customer");
        }
        update_default_text();
        return;
    }
    if($('.one-per-customer').prop("checked")) $('.user-limit-text').text("One use per customer");
    else $('.user-limit-text').text("");
    update_default_text();
}
...
function change_Eligibility_text() {
    if($('input[name="eligibility"][value="all-customers"]').prop("checked")) $('.customer-eligibility-text').text("For all customers");
    else if($('input[name="eligibility"][value="all-employees"]').prop("checked")) $('.customer-eligibility-text').text("For all employees");
    else if($('input[name="eligibility"][value="employees"]').prop("checked")) {
        let rows_cnt = $('#employee-table').find('tbody>tr').length;
        if(rows_cnt == 0) $('.customer-eligibility-text').text("For specific employees");
        else if(rows_cnt == 1) $('.customer-eligibility-text').text("For " + $('#employee-table').find('tbody>tr').eq(0).find('td').eq(0).find('b').text());             
        else $('.customer-eligibility-text').text("For " + rows_cnt + " employees");
    }
    else if($('input[name="eligibility"][value="customers"]').prop("checked")) {
        let rows_cnt = $('#customer-table').find('tbody>tr').length;
        if(rows_cnt == 0) $('.customer-eligibility-text').text("For specific customers");
        else if(rows_cnt == 1) $('.customer-eligibility-text').text("For " + $('#customer-table').find('tbody>tr').eq(0).find('td').eq(0).find('b').text());             
        else $('.customer-eligibility-text').text("For " + rows_cnt + " customers");
    }
}
...
function change_Discount_type(ev) {
    ...
    } else if($(ev).attr("name") == "eligibility") {
        if($(ev).val() == "employees") {
            $('#employee-search-div').removeClass("d-none");
            $('#customer-search-div').addClass("d-none");
        }
        else if($(ev).val() == "customers") {
            $('#employee-search-div').addClass("d-none");
            $('#customer-search-div').removeClass("d-none");
        } else {
            $('#employee-search-div').addClass("d-none");
            $('#customer-search-div').addClass("d-none");
        }
...
function change_Limit_number(ev) {
    if($(ev).hasClass("limit-times")) {
        if($(ev).prop("checked")) {
            $('.limit-total').removeClass("d-none");
        } else $('.limit-total').addClass("d-none");
    }
}

...
(function($){
    ...
    $.fn.email_multiple = function(options) {
        ...
        {
            $(this).after(
                "<div class=\"all-mail\" style=\"width: 100%;\"></div>\n" +
                "<input type=\"text\" name=\"email\" class=\"enter-mail-id email-input-control form-control\" placeholder=\"Emails separated by space or enter ...\" />");
            let $orig = $(this);
            let $element = $('.enter-mail-id');
            $element.keydown(function (e) {
                $element.css('border', '');
                if (e.keyCode === 13 || e.keyCode === 32) {
                    let getValue = $element.val().replace(' ','');
